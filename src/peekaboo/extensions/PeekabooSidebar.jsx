...
const Peekaboo =  ({}) => {
  ...
  return (
    <Card>
    <Panel id="panel-1" width='lg' onClose={() => {
    ...
    <ButtonRow>
      <Button disabled={!!retainedReactions} variant="primary" onClick={async (event, reactions) => {
        retain(reactions)
        setRetainedReactions(reactions)
      }}>
        Play Peek-a-boo!
      </Button>
      <Button disabled={!retainedReactions} variant="secondary" onClick={async (event, reactions) => {
        clearInterval(peekabooInterval)
